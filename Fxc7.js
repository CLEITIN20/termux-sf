...
async function starts() {
	...
	frhan.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `iae membro novo @${num.split('@')[0]} \nBem vindo ao grupo *${mdata.subject}*\nsiga as regras para evitar de ser banido \ncomando do bot (.menu ou .help .infobot)\nnome?\nidade?\nsexo masculino ou feminino?\nfoto do cartão de crédito frente e verso pfv e o cpf tmb\nadiciona o número do criador do bot wa.me/5527998401752!!`
				...
				teks = `tchau fdp @${num.split('@')[0]} não foi um prazer te ver aqui`
				let buff = await getBuffer(ppimg)
				frhan.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	frhan.on('chat-update', async (mek) => {
		try {
			...
			const FarhanGans2 = " ~ criado por henrique Soares			const isUrl = (url) => {
...
if (isGroup) {
		...
		setTimeout ( () => {
		frhan.sendMessage(from, `Desculpe Yaa Tchau👋`, text)
		}, 1000)
		setTimeout( () => {
		frhan.sendMessage(from, `Desculpa ${name} Não é possível entrar no grupo por causa de membros do grupo ${groupMetadata.subject} Limite de Membro Não Cumprindo\n\nMinimal Member ${memberLimit}`, text)
		}, 0)
	}
		...
		setTimeout( () => {
		reply(`Link no grupo detectado, desculpe *${pushname2}* você será expulso chameno wa.me/5527998401752 para voltar`)
		}, 0)
	}
		if (txt.length > 500){
        if (!isGroup) return
		if (!isAntiVirtex) return
		if (isGroupAdmins) return reply(`${pushname2} Seu administrador de grupo não será chutado`)
		frhan.updatePresence(from, Presence.composing)
		var kic = `${sender.split("@")[0]}@s.whatsapp.net`
		costum(monospace(`Virtex detectado, desculpe ${sender.split("@")[0]} você será expulso do grupo`))
		setTimeout( () => {
		...
		if (!isGroupAdmins) {
		return reply("MANTENHA A FALA DE DONG!!")
		.then(() => frhan.groupRemove(from, sender))
		.then(() => {
		frhan.sendMessage(from, `*「 ANTI BADWORD 」*\Você foi chutado por falar asperamente!`, text ,{quoted: mek})
		}).catch(() => frhan.sendMessage(from, `Felizmente, iren não é admin, se o admin já o chutou!`, text , {quoted : mek}))
		} else {
...
const checkLimit = (sender) => {
			...
			frhan.sendMessage(from, `seu limite : ${limitCounts}`, text, { quoted : mek})
			found = true 
			}
		}
			if (found === false) {
...
const obj = { id: sender, limit: 0 }
			...
			expiredCheck()
			var premi = '*NÃO PREMIUM*'
			if (isPrem) {
			premi = '*SIM PREMIUM*'
			} 
			if (isOwner) {
			premi = '*ILIMITADO PREMIUM*'
			}
			var Simihh = 'off'
			if(isSimi) {
			Simihh = 'on'
...
return new Promise(resolve => setTimeout(resolve, ms));
		...
		break
			case 'vapor':
				...
				case 'pitch':
					if (!isOwner) return reply(mess.only.owner)
					if (!isQuotedAudio) return reply('som de resposta!!!')
					...
					if (args[0] > 12) return reply("Max 12 letras")
...
// end fitur simple
			...
			case 'infobot':
				...
				if (!isUser) return reply(mess.only.userB)
					...
					frhan.sendMessage(from, `bye${body.slice(9)}@c.us`, text)
				    break 
				case 'readmore':
					if (isBanned) return reply(mess.only.benned)    
					if (!isUser) return reply(mess.only.userB)
				...
				reply(`LIMIT TRABALHOS AO RESET`)
				break
				case 'limit':
					if (!isUser) return reply(mess.only.userB)
					checkLimit(sender)
			...
			break
				...
				if (!isGroup) return reply(mess.only.group)
				if (!isOwner) return reply(mess.only.ownerB)
				anu = await frhan.groupLeave(from, `Tchau, todos os membros *${groupMetadata.subject}*`, groupId)
				break
				case 'setname':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
			...
			case 'setdesc':
				...
				frhan.sendMessage(from, `\`\`\`✓Alteração das descrições do grupo com sucesso\`\`\` *${groupMetadata.subject}* Menjadi: *${body.slice(9)}*`, text, {quoted: mek})
			break
			case 'tts':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
			...
			case 'nsfwloli':
				...
				frhan.sendMessage(from, buffer, image, {quoted: mek, caption: 'Não faça ingredientes para o tio comum'})
				} catch (e) {
...
//creator
			...
			case 'ttp':
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
				if (args.length < 1) return reply('que adesivo de texto fazer?')
				reply(mess.wait)
				try {
					ranp = getRandom('.png')
					rano = getRandom('.webp')
...
//akhir kreator
		    ...
		    try {
			    ...
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔═══✪ Marcado por *${pushname2}* ✪══`+ teks +'╚═══〘 HENRIQUE BOT 〙═══', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 HENRIQUEBOT 〙', members_id, true)
					break
			    case 'kbbi':
			    try {
			    if (isBanned) return reply(mess.only.benned)    
				...
				case 'kicktime':
					...
					setTimeout( () => {
					frhan.sendMessage(from, 'Asikkk Get Food nihh:D', text)
					}, 2500)
					setTimeout( () => {
				...
				case 'antivirtex':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('digite para ativar')
					if ((args[0]) === 'on') {
				...
				if (!isGroupAdmins) return reply(mess.only.admin)
					...
					if ((args[0]) === 'on') {
						...
						if (!isSimi) return reply('O modo Simi já foi desativado antes')
				...
				if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('digite! badword ativado para ativar')
					if ((args[0]) === 'on') {
